...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_FLOAT:
    eat(TK_FLOAT);
    constValue = makeFloatConstant(currentToken->f_value);
    break;
  case TK_IDENT:
    eat(TK_IDENT);
...
Type* compileExpression(void) { // TYPE*
  Type* type;
...
Type* compileExpression2(void) { // TYPE*
  Type* type;
...
void compileExpression3(void) { // TYPE*
  Type* type;
...
Type* compileTerm(void) { // TYPE*
  Type *type, *type2;

  type = compileFactor();
  type2 = compileTerm2(type);
